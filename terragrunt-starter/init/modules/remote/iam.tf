...
resource "aws_iam_policy" "s3_bucket_rw_access" {
  name        = "${var.application_name}-s3-${var.s3_bucket_name}-rw-access"
...
resource "aws_iam_policy" "s3_bucket_ro_access" {
  name        = "${var.application_name}-s3-${var.s3_bucket_name}-ro-access"
...
resource "aws_iam_policy" "dynamodb_table_access" {
  name        = "${var.application_name}-dynamodb-${var.dynamodb_table_name}-access"
...
resource "aws_iam_policy" "kms_sops_access" {
  name        = "${var.application_name}-kms-plan-access"
  description = "Grants Decrypt access to the KMS key used for sops encryption"
  policy      = data.aws_iam_policy_document.kms_sops_decrypt.json
}
...
resource "aws_iam_group" "terraform_rw_access" {
  name = "${var.application_name}-${var.iam_group_name_rw_access}"
}

resource "aws_iam_group" "terraform_ro_access" {
  name = "${var.application_name}-${var.iam_group_name_ro_access}"
}

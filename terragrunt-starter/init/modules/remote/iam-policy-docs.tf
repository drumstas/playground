...
data "aws_iam_policy_document" "s3_bucket_rw_access" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::${var.application_name}-${var.s3_bucket_name}*",
    ]
  }
...
data "aws_iam_policy_document" "dynamodb_table" {
  statement {
    ...
    resources = [
      "arn:aws:dynamodb:${var.aws_region}:${data.aws_caller_identity.current.account_id}:table/${var.application_name}-${var.dynamodb_table_name}",
    ]
  }
}

data "aws_iam_policy_document" "kms_sops_decrypt" {
  statement {
    actions = ["kms:Decrypt"]
    resources = [
      aws_kms_key.encryption_key_sops.arn
    ]
  }
}
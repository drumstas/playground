...
public class ShowEntity extends EventSourcedBehaviorWithEnforcedReplies<ShowEntityCommand, ShowEvent, Show> {
    ...
    public static Behavior<ShowEntityCommand> create(ShowId showId, Clock clock) {
        return Behaviors.setup(context -> {
            PersistenceId persistenceId = PersistenceId.of("Show", String.valueOf(showId.id()));
    ...
    private ReplyEffect<ShowEvent, Show> handleShowCommand(Show show, ShowEntityCommand.ShowCommandEnvelope envelope) {
        ...
        return show.process(command, clock).fold(
                error -> {
                    ...
                    return Effect().reply(envelope.replyTo(), new ShowEntityResponse.CommandRejected(error));
                },
                events -> {
                    context.getLog().debug("Command handled: {}", command);
                    return Effect().persist(events.asJava())
                            .thenReply(envelope.replyTo(), s -> new ShowEntityResponse.CommandProcessed());
                }
        );
    }
